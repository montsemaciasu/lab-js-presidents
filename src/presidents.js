...
function countYearsInOffice(presidentsArr) {
  const total = presidentsArr.reduce((acc, president) => {
    if (president.leftOffice) {
      let years = president.leftOffice - president.tookOffice;
      return acc + years;
    }
    return acc;
  }, 0);
...
function sortPresidentsByBirthYear(presidentsArr) {
  return presidentsArr.sort(
    (presidentA, presidentB) => presidentA.birthYear - presidentB.birthYear
  );
}
...
function sortPresidentsByName(presidentsArr) {
  return presidentsArr.sort(
    (presidentA, presidentB) => presidentB.name < presidentA.name
  );
}
